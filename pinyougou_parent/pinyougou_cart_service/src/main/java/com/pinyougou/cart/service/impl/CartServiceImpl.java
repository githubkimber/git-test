...
public class CartServiceImpl implements CartService {
    ...
    public List<Cart> findCartListFromRedis(String username) {
        ...
        System.out.println("向redis中提取购物车"+username);
    ...
    public TbOrderItem searchOrderItemByItemId(List<TbOrderItem> orderItemList, Long itemId){
        for (TbOrderItem orderItem:orderItemList){
            // 首字母大写LongValue()比较对象地址,首字母小写longValue()比较值,
    ...
    private TbOrderItem createOrderItem(TbItem item, Integer num ){
        ...
        orderItem.setGoodsId(item.getGoodsId()); // 添加购物车Id
        orderItem.setItemId(item.getId());
        orderItem.setNum(num);
        orderItem.setPicPath(item.getImage());  // 添加图片地址
