...
public class TrafficUtil {
     ...
     * @return
     */
    public static Map<String, Integer> trafficLinkInfo(String strFlage){  //strFlage  就是CSANTI_MONITOR_LP 
    	//实例化redis集群    （读取配置志文件内的配置信息）
        JedisCluster jedisCluster = JedisConnectionUtil.getJedisCluster();
        //从redis中拿出所有以CSANTI_MONITOR_LP作为前缀的key
        Set<String> keySet = JedisConnectionUtil.keys(jedisCluster,strFlage + "*");
       
        //转换为可操作性的bean
        List<LinkJsonVO> jsonVOs = new ArrayList<LinkJsonVO>();
        //记录当天的链路总数  ！！！    总统计   Map<192.168.2.141,100>
    	Map<String, Integer> linkCount = new HashMap<String, Integer>();
    	 try {
        	//循环所有key
            for(String str : keySet){   //str  CSANTI_MONITOR_LP1553088805099
            	LinkJsonVO jsonVO = new LinkJsonVO();
            		//获得某一个key 对应的value    ({"serversCountMap":{"192.168.2.141":30},"activeNumMap":{"192.168.2.141":"5"}})
                String value = jedisCluster.get(str);
                	// 将 {"serversCountMap":{"192.168.2.141":30},"activeNumMap":{"192.168.2.141":"5"}}  转换成LinkJsonVO类的bean
                jsonVO = JsonResolveUtil.resolveLinkJson(value);
                	//获取jsonVO Bean 中的   serversCountMap ，其中包含多个serveraddress  -->{"192.168.2.141":35}
                Map<String, Integer> serversCountMap = jsonVO.getServersCountMap();
                	//获取每个serversCountMap中的IP
                Set<String> keySet2 = serversCountMap.keySet();
                	//遍历每个IP string就是每一个IP 
                for (String string : keySet2) {  
                		//先从linkCount中获取这个IP 的值，
                	...
                	Integer orDefault2 = serversCountMap.get(string);
                		//将 总统计linkCount 内的值和这个IP的值求和，并推送到 总统计变量中
